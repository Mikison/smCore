...
public class ClearCommand
{
    ...
    private int clearInventory(CommandContext<CommandSourceStack> context, Player target)
    {
        ...
        if (target == null)
        {
            ...
            if (!PlayerUtil.playerHasPermission(player, "smcore.clear"))
                return 0;
            player.getInventory().clear();
            player.sendMessage(MM."<bold><dark_gray>[<blue>!<dark_gray>]</bold> <gray>Your inventory has been cleared");
            return Command.SINGLE_SUCCESS;
        }
        if (!PlayerUtil.playerHasPermission(sender, "smcore.clear.other"))
            return 0;

        target.getInventory().clear();
        target.sendMessage(
                MM."<bold><dark_gray>[<blue>!<dark_gray>]</bold> <gray>Your inventory has been cleared by \{PlayerUtil.getPlayerNameWithRank(
                        sender)}");
        sender.sendMessage(
                MM."<bold><dark_gray>[<blue>!<dark_gray>]</bold> <gray>You have cleared the inventory of \{PlayerUtil.getPlayerNameWithRank(
                        target)}");
