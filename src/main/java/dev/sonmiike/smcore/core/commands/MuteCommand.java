...
public class MuteCommand {
    ...
    private void registerMute(SmCore plugin, Commands commands) {
        final LiteralArgumentBuilder<CommandSourceStack> muteBuilder = Commands.literal("mute")
            .then(Commands.argument("player", ArgumentTypes.player())
                .then(Commands.argument("duration", new MuteArgument())
                    .executes(ctx -> {
                        CommandSender sender = ctx.getSource().getSender();
                        if (!PlayerUtil.playerHasPermission(sender, "smcore.mute")) return 0;
    ...
    private void executeMute(CommandSourceStack sourceStack, Player target, MuteArgument.DurationPair durationPair, String reason) {
        ...
        Component muteMessage = MM."<bold><dark_gray>[<red>!<dark_gray>]</bold> <gray>\{target.getName()}<red> got muted by \{PlayerUtil.getPlayerNameWithRank(sender)} <red>for <white>\{humanReadableDuration} ";
    ...
    private void registerUnMute(SmCore plugin, Commands commands) {
        final LiteralArgumentBuilder<CommandSourceStack> unmuteBuilder = Commands.literal("unmute")
            .then(Commands.argument("player", ArgumentTypes.player())
                .executes(ctx -> {
                    CommandSender sender = ctx.getSource().getSender();
                    if (!PlayerUtil.playerHasPermission(sender, "smcore.unmute")) return 0;
    ...
    private void executeUnMute(CommandSourceStack sourceStack, Player target) {
        ...
        if (!muteManager.isPlayerMuted(target.getUniqueId())) {
            sender.sendMessage(MM."<bold><dark_gray>[<red>!<dark_gray>]</bold> <gray>\{target.getName()}<red> is not muted!");
            return;
        }
        sender.sendMessage(MM."<bold><dark_gray>[<red>!<dark_gray>]</bold> <gray>\{target.getName()}<red> got unmuted!");
        Bukkit.getOnlinePlayers().forEach(player -> player.sendMessage(MM."<bold><dark_gray>[<red>!<dark_gray>]</bold> <gray>\{target.getName()}<red> was unmuted by \{PlayerUtil.getPlayerNameWithRank(sender)}"));
