...
public class TeleportCommand
{
    ...
    private void register(JavaPlugin plugin, Commands commands)
    {
        final LiteralArgumentBuilder<CommandSourceStack> teleportBuilder = Commands.literal("tp")
                .then(Commands.argument("player", ArgumentTypes.player()).executes(ctx -> teleportToPlayer(ctx,
                        ctx.getArgument("player", PlayerSelectorArgumentResolver.class).resolve(ctx.getSource())
                                .getFirst()))).then(Commands.argument("x", IntegerArgumentType.integer())
                        .then(Commands.argument("y", IntegerArgumentType.integer())
                                .then(Commands.argument("z", IntegerArgumentType.integer()).executes(
                                        ctx -> teleportToCoordinates(ctx, ctx.getArgument("x", Integer.class),
                                                ctx.getArgument("y", Integer.class),
                                                ctx.getArgument("z", Integer.class))))))
                .then(Commands.argument("player1", ArgumentTypes.player())
                        .then(Commands.argument("player2", ArgumentTypes.player()).executes(
                                ctx -> teleportPlayerToPlayer(ctx,
                                        ctx.getArgument("player1", PlayerSelectorArgumentResolver.class)
                                                .resolve(ctx.getSource()).getFirst(),
                                        ctx.getArgument("player2", PlayerSelectorArgumentResolver.class)
                                                .resolve(ctx.getSource()).getFirst()))));
    ...
    private int teleportToPlayer(CommandContext<CommandSourceStack> context, Player target)
    {
        ...
        if (sender instanceof Player player)
        {
            ...
            sender.sendMessage(
                    MM."<bold><dark_gray>[<blue>!<dark_gray>]</bold> <gray>Teleported to \{PlayerUtil.getPlayerNameWithRank(
                            target)}");
        }
        else
        {
    ...
    private int teleportToCoordinates(CommandContext<CommandSourceStack> context, int x, int y, int z)
    {
        ...
        if (sender instanceof Player player)
        {
            ...
            sender.sendMessage(
                    MM."<bold><dark_gray>[<blue>!<dark_gray>]</bold> <gray>Teleported to <bold>X: <blue>\{x} <gray>Y: <blue>\{y} <gray>Z: <blue>\{z}");
        }
        else
        {
            sender.sendMessage(
    ...
    private int teleportPlayerToPlayer(CommandContext<CommandSourceStack> context, Player player1, Player player2)
    {
        ...
        context.getSource().getSender().sendMessage(
                MM."<bold><dark_gray>[<blue>!<dark_gray>]</bold> <gray>Teleported \{PlayerUtil.getPlayerNameWithRank(
                        player1)} to \{PlayerUtil.getPlayerNameWithRank(player2)}");
        return Command.SINGLE_SUCCESS;
    }
}
