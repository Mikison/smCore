...
public class FlyCommand
{
    ...
    {
        final LiteralArgumentBuilder<CommandSourceStack> flyBuilder = Commands.literal("fly").executes((source) -> {
            ...
            if (!PlayerUtil.playerHasPermission(player, "smcore.fly"))
                return 0;
            player.setAllowFlight(!player.getAllowFlight());
            player.sendMessage(
                    MM."<bold><dark_gray>[<blue>!<dark_gray>]</bold> <gray>Fly mode <blue>\{(player.getAllowFlight() ?
                            "ENABLED" :
                            "DISABLED")}");
            ...
            if (!PlayerUtil.playerHasPermission(target, "smcore.fly.other"))
                return 0;
            target.setAllowFlight(!target.getAllowFlight());
            target.sendMessage(
                    MM."<bold><dark_gray>[<blue>!<dark_gray>]</bold> <gray>Fly mode <blue>\{(target.getAllowFlight() ?
                            "ENABLED" :
                            "DISABLED")} <gray>by \{PlayerUtil.getPlayerNameWithRank(sender)}");
            context.getSource().getSender().sendMessage(
                    MM."<bold><dark_gray>[<blue>!<dark_gray>]</bold> <gray>You have \{(target.getAllowFlight() ?
                            "ENABLED" :
                            "DISABLED")} fly mode for \{PlayerUtil.getPlayerNameWithRank(target)}");
            return Command.SINGLE_SUCCESS;
        }));
