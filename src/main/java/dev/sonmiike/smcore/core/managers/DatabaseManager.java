...
public class DatabaseManager
{
    ...
    public UUID getUUIDFromUsername(String username)
    {
        try (Connection connection = dataSource.getConnection())
        {
            PreparedStatement statement = connection.prepareStatement("SELECT uuid FROM players WHERE playerName = ?");
    ...
    public String getUsernameFromUUID(UUID uuid)
    {
        try (Connection connection = dataSource.getConnection())
        {
            PreparedStatement statement = connection.prepareStatement("SELECT playerName FROM players WHERE uuid = ?");
    ...
    public void updateMuteStatus(UUID uuid, boolean isActive)
    {
        try (Connection connection = dataSource.getConnection())
        {
            PreparedStatement statement = connection.prepareStatement("UPDATE mutes SET is_active = ? WHERE uuid = ?");
    ...
    {
        String playerSql = "CREATE TABLE IF NOT EXISTS players (" + "uuid VARCHAR(50) NOT NULL PRIMARY KEY,"
                + "playerName VARCHAR(16) NOT NULL," + "first_join TIMESTAMP NOT NULL,"
                + "last_join TIMESTAMP NOT NULL," + "ip_address VARCHAR(16) NOT NULL" + ")";
        executeUpdate(playerSql, _ -> {
        });

        String bansSql = "CREATE TABLE IF NOT EXISTS bans (" + "id SERIAL PRIMARY KEY," + "uuid VARCHAR(50) NOT NULL,"
                + "reason TEXT NOT NULL," + "banned_by VARCHAR(16) NOT NULL," + "ban_date TIMESTAMP NOT NULL,"
                + "expires_at TIMESTAMP," + "is_active BOOLEAN NOT NULL DEFAULT TRUE,"
                + "FOREIGN KEY (uuid) REFERENCES players(uuid)" + ")";
        executeUpdate(bansSql, _ -> {
        });

        String mutesSql = "CREATE TABLE IF NOT EXISTS mutes (" + "id SERIAL PRIMARY KEY," + "uuid VARCHAR(50) NOT NULL,"
                + "reason TEXT," + "muted_by VARCHAR(16) NOT NULL," + "mute_date TIMESTAMP NOT NULL,"
                + "expires_at TIMESTAMP," + "is_active BOOLEAN NOT NULL DEFAULT TRUE,"
                + "FOREIGN KEY (uuid) REFERENCES players(uuid)" + ")";
